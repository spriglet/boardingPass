...
var click_sensei_card = function click_sensei_card() {
...
var create_class = function create_class(response) {
    return false;
};
...
var post_request = function post_request(formId, mappings, url, response_handler) {
...
var build_sensei_list = function build_sensei_list(url) {
    ...
    http.GET(url, function (data) {
        ...
        data.forEach(function (row) {
            ...
            var sensei = row.user.id;
            var image = row.image_url;
            var profile_card = templates.profile(username, firstName, lastName, country, image);
            var content = templates.list.row(profile_card, '/sensei/profile/' + sensei, sensei);
...
var create_page = function create_page(page, assignEvents) {
...
list.row = function (content, link, data) {
                console.log(link);
                var html = '  <a href="' + link.toString() + '" class="list-group-item" data="' + data + '">\n' + '    <div class="d-flex w-100 justify-content-between">\n' + content + '    </div>\n' + '  </a>\n';
                return html;
};

var profile = function profile(username, firstName, lastName, country, img) {
...
var lesson_card = function lesson_card(index, id, name, description, max_students, length_in_minutes, cost, currency) {
                var html = '<div class="card card-block">\n' + '    <h4 class="card-header">' + name + '</h4>\n' + '    <p class="card-text">' + description + '</p>\n' + '    <a href="#" id="lesson' + index + '" class="lesson btn btn-primary">Button Primary</a>\n' + '</div>';
...
var properties = {
    landing: {
        ...
        content: [{
            id: 'signup',
            mappings: { username: "username", password: "password", password_confirm: "confirm_password", email: "email", first_name: "first_name", last_name: "last_name" },
            ajaxUrl: '/api/v1/accounts/register/',
            isForm: true,
            post: events.post_request,
            response_handler: events.sign_up,
            rules: {
                username: validation.username.rules,
                first_name: validation.first_name.rules,
                last_name: validation.last_name.rules,
                password: validation.password.rules,
                confirm_password: validation.confirm_password.rules,
                email: validation.email.rules
            },
            messages: {
                username: validation.username.messagse,
                first_name: validation.first_name.messages,
                last_name: validation.last_name.messages,
                password: validation.password.messages,
                confirm_password: validation.confirm_password.messages,
            ...
            id: 'login_form',
            isForm: true,
            ajaxUrl: '/api/v1/accounts/login/',
            mappings: { login: "username", password: "password" },
            post: events.post_request,
            response_handler: events.login,
            rules: {
                username: validation.username.rules,
                password: validation.password.rules
            ...
            messages: {
                username: validation.username.messages,
                password: validation.password.messages
            }
    ...
    sensei_list: {
        ...
        content: [{ id: 'sensei',
            ajaxUrl: '/rest/sensei/',
            get_request: events.build_sensei_list
        }]

    },
    sensei_profile: {
        id: null,
        templateUrl: '/sensei/profile/',
        content: [{ id: 'class_list', ajaxUrl: "", get_request: null }]
    }, scheduler: {
        id: null,
        ...
        content: [{
            ...
            rules: {
                name: validation.name.rules,
                description: validation.description.rules,
                seat_cost: validation.seat_cost.rules,
                student_threshold: validation.student_threshold.rules

            },
            messages: {
                name: validation.name.messages,
                description: validation.description.messages,
                seat_cost: validation.seat_cost.messages,
                student_threshold: validation.student_threshold.messages
            }
...
function create(pageName, events_handlers) {
    events.create_page(properties[pageName], events_handlers);
}
...
var page = __webpack_require__(/*! ./boardingPass/pages */ "./boardingPass/pages.js");
global._ = __webpack_require__(/*! underscore */ "./node_modules/underscore/underscore.js");
